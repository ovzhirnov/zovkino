...
{ ?>
Работаем с сайтом: http://vimaxfilms.ru
...
else {echo "не могу прочитать страницу<br>";}
$save_url='';
$rf=trim(chop($url_readfile));
while (strstr($rf,"<div class=\"st_next\"><a href=\""))//начальная строка для поиска
{
$s1=strpos($rf,"<div class=\"st_next\"><a href=\"",0);//начальная строка для поиска
if ($s1)
{
$s2=strpos($rf,"\">Подр",$s1);//конечная строка для поиска
$s=$s2-$s1;
...
$strneedle=array ("",">Год выхода</span>:",">Название</span>: <strong>",">Оригинальное название</span>: ",">Жанр</span>: ",">Режиссер</span>: ",">В ролях</span>:",">Продолжительность</span>: ",":</strong><br>",">Качество:",">Озвучивание</span>: ",">Видео: ",">Аудио: ","(Размер: ~");
...
else {echo "не могу прочитать страницу по url из списка<br>";}

for ($i=1;$i<=(count($strneedle)-1);$i++)
{
$rf=trim(chop($readfile));
$s1=strpos($rf,$strneedle[$i],0);
	if ($i==1 && !$s1) {$strneedle[$i]=">Год выпуска</span>:"; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==1 && !$s1) {$strneedle[$i]=">Год выпуска:</span> "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==1 && !$s1) {$strneedle[$i]=">Год выхода:</span> "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==1 && !$s1) {$strneedle[$i]=">Год выпуска: </span>"; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==2 && !$s1) {$strneedle[$i]=">Название фильма:</span> <strong>"; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==2 && !$s1) {$strneedle[$i]=">Название фильма</span>: <strong>"; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==2 && !$s1) {$strneedle[$i]=">Название:</span> <strong>"; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==2 && !$s1) {$strneedle[$i]=">Название</span>:  <strong>"; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==3 && !$s1) {$strneedle[$i]=">Оригинальное название:</span> "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==4 && !$s1) {$strneedle[$i]=">Жанр:</span> "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==5 && !$s1) {$strneedle[$i]=">Режиссёр</span>: "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==5 && !$s1) {$strneedle[$i]=">Режиссер:</span> "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==5 && !$s1) {$strneedle[$i]=">Режессёр</span>: "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==5 && !$s1) {$strneedle[$i]=">Режисер</span>: "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==6 && !$s1) {$strneedle[$i]=">В ролях:</span> "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==6 && !$s1) {$strneedle[$i]=">Артисты</span>: "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==7 && !$s1) {$strneedle[$i]=">Продолжительность:</span> "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==8 && !$s1) {$strneedle[$i]="О фильме ".$wdata[2]; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==8 && !$s1) {$strneedle[$i]="Описание фильма ".$wdata[2]; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==8 && !$s1) {$strneedle[$i]=":</strong> <br />".$wdata[2]; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==9 && !$s1) {$strneedle[$i]="> Качество:"; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==10 && !$s1) {$strneedle[$i]=">Перевод</span>: "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==10 && !$s1) {$strneedle[$i]=">Перевод:</span> "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==10 && !$s1) {$strneedle[$i]=">Озвучивание:</span> "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==11 && !$s1) {$strneedle[$i]="> Видео: "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==12 && !$s1) {$strneedle[$i]="<li>Звук: "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==12 && !$s1) {$strneedle[$i]="> Аудио: "; $s1=strpos($rf,$strneedle[$i],0);}
	if ($i==12 && !$s1) {$strneedle[$i]="<li>Аудио 1: "; $s1=strpos($rf,$strneedle[$i],0);}
if ($s1)
{
$s2=strpos($rf,$strend[$i],$s1);
$s=$s2-$s1;
$rf=substr($rf,$s1,$s);
$wdata[$i]=str_replace($strneedle[$i],"",$rf);//данные для записи
