...
{ ?>
Работаем с сайтом: http://kinolive.net
...
while (strstr($rf,"align=\"center\">
   <a href=\""))//начальная строка для поиска
{
$s1=strpos($rf,"align=\"center\">
   <a href=\"",0);//начальная строка для поиска
$s2=strpos($rf,"\" title=",$s1);//конечная строка для поиска
$s=$s2-$s1;
...
$strneedle=array ("","Год выхода:</span>","Название ленты:</span>","Оригинальное название:</span>","Жанр:</span>","Режиссёр:</span>","В главных ролях:</span>","Продолжительность:</span>","О сюжете фильма","<li>Качество:","<li>Перевод ","<li>Видео:","<li>Аудио:","<li>Размер скачиваемого файла:");
...
if ($row1['orig_name']==$wdata[3])
	{echo "<br>совпадение 100% ";} else {echo "<br>совпадение 50% ";}
...
$rf=trim(chop($readfile));
$s1=strpos($rf,"\" alt=\"Скачать".$wdata[2],0);
if ($s1<>"")
{
...
/*$picurl=str_replace("src=\"","",$rf);//готовый относительный адрес картинки
$picurl=str_replace("/uploads/posts/","",$rf);//готовый относительный адрес картинки
$picurl=str_replace("src=\"/uploads/posts/","",$rf);//готовый относительный адрес картинки
*/
$picurl=$rf;
$zz=strrpos($picurl,"/");
...
//готовим адрес страницы со ссылкой letitbit
$rf=trim(chop($readfile));
$s1=strpos($rf,"\">скачать фильм</a>",0);
$s2=strpos($rf,"<a href=\"",$s1-100);
$s=$s1-$s2;
$rf=substr($rf,$s2,$s);
$pageletiturl=str_replace("<a href=\"","",$rf);//адрес страницы со ссылкой letitbit
...
$rf=substr($rf,$s1,$s);
$letiturl=$rf;
//$letiturl=str_replace("Ссылка 1 (основная) : [ <a href=\"","",$rf);//готовый адрес ссылки letitbit
