...
{ ?>
Работаем с сайтом: http://filmera.net
...
while (strstr($rf,"<div class=\"photos\"><a href=\""))//начальная строка для поиска
{
$s1=strpos($rf,"<div class=\"photos\"><a href=\"",0);//начальная строка для поиска
$s2=strpos($rf,"\">",$s1+30);//конечная строка для поиска
$s=$s2-$s1;
$stemp=substr($rf,$s1,$s);
$stemp=str_replace("<div class=\"photos\"><a href=\"",'',$stemp);
$save_url .=$stemp."\n";
$rf=substr($rf,($s2+10));
}
$fp=fopen($file_s_url,'w+');
...
$strneedle=array ("","Год выпускa:</b>","#ff0000; font-size: large;\"><br>","Название (оригинал):</b>","Жaнp:</b>","Режисcер:</b>","Актeры:</b>","Прод.:</b>","&#959;пис&#945;ние фильм&#945;:</b>","Формaт:</b>","Перевoд:</b>","Видeo:</b>","3вyk:</b>","Рaзм.:</b>");
$strend=array ("","<br>","</span><br>","<br>","<br>","<br>","<br>","<br>","</div>","<br>","<br>","<br>","<br>","<br>");
$file=fopen($link,'rb');
if ($file)
{
$readfile = '';
...
$s1=strpos($rf,"onClick=\"return hs.expand(this)\" ><img align=\"left\" src=\"",0);
if ($s1<>"")
{
$s2=strpos($rf,"\" style=\"border:",$s1);
$s=$s2-$s1;
$rf=rawurldecode(substr($rf,$s1,$s));
}

else {$rf="";}

$rf=str_replace("onClick=\"return hs.expand(this)\" ><img align=\"left\" src=\"","",$rf);
...
$rf = @base64_decode ( $rf );
$rf = str_replace ( "&amp;", "&", $rf );
$rf = str_replace ("-FilmEra.Net", "", $rf );
$rf = str_replace ("__www.filmera.net", "", $rf );
